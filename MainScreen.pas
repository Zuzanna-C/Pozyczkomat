...
type
  TForm2 = class(TForm)
    ...
    procedure OnClickT(Sender: TObject);
  private
  ...
  with TForm5.Create(Self) do
    try
      ...
      begin
        ...
        DataModule1.FDQueryAllTransactions.ParamByName('uid').AsInteger
          := userID;
...
begin
  ...
  begin
    ShowMessage
      ('Nie masz uprawnień do edycji tego rekordu, ponieważ został on utworzony przez innego użytkownika.');
    Exit;
  end;
      ...
      begin
        case mode of
          ...
          tmBorrowed:
            begin
              DataModule1.FDQueryBorrowed.Close;
              DataModule1.FDQueryBorrowed.ParamByName('uid').AsInteger
                := userID;
              DataModule1.FDQueryBorrowed.Open;
            end;
        end;
...
begin
  ...
  begin
    ...
    try
      DataModule1.FDQueryBorrowed.Close;
      DataModule1.FDQueryDeleteBorrowing.ParamByName('id').AsInteger :=
        recordID;
...
var
  i: Integer;
begin
  // Wyzeruj wszystkie kontrolki (ComboBox, CheckBox, Edit)
  ...
  // Wyzeruj wartości w lookupach
  DBLCBEntities.KeyValue := Null;
  DBLCBTypes.KeyValue := Null;

  // Usuń filtry na wszystkich datasetach
  with DataModule1 do
  begin
    FDQueryLoans.Filtered := False;
    FDQueryBorrowed.Filtered := False;
    FDQueryAllTransactions.Filtered := False;

    FDQueryLoans.Filter := '';
    FDQueryBorrowed.Filter := '';
    FDQueryAllTransactions.Filter := '';
  end;
...
procedure TForm2.OnClickT(Sender: TObject);
begin
  UpdateFilter;
end;
...
begin
  ...
  begin
    ...
    filterText := filterText + 'TypeID = ' +
      IntToStr(Integer(DBLCBTypes.KeyValue));
  end;
  ...
  begin
    filterText := filterText + 'Returned = 0';
