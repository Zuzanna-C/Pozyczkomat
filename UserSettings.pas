...
begin
  ...
  begin
    ...
    if MessageDlg('Deactivate this type?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
    then
    begin
  ...
  begin
    ...
    begin
      ShowMessage
        ('Nie można dezaktywować tej encji – jest powiązana z użytkownikiem.');
      Exit;
    end;

    if MessageDlg('Deactivate this entity?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
    then
    begin
...
begin
  ...
  begin
    ShowMessage
      ('Nie można edytować tej encji – jest powiązana z użytkownikiem.');
    Exit;
  end;

  EditEditEntity.Text := DataModule1.FDQueryEntities.FieldByName
    ('Name').AsString;
...
begin
  ...
  EditEditType.Text := DataModule1.FDQueryTypes.FieldByName('TypeName')
    .AsString;
...
function GetCurrentCredentials(UserID: Integer;
  out CurrentUsername, CurrentPassword: string): Boolean;
begin
  Result := False;
  with DataModule1.FDQueryGetUserData do
  begin
...
begin
  ...
  DataModule1.FDQueryAddType.Params.ParamByName('TypeName').AsString :=
    EditAddType.Text;
